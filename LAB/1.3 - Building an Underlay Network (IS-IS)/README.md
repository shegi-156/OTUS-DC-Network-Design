...
Интерфейс lo0 в режиме passive (на интерфейсах с которых строится соседтсво не включается)

Максимальное значение балансировки по ecmp выбрано 4 (с учетом возможной модернизации и добавления новых SPINE)

BFD включен для всех интерфейсов учевствующих в процессе IS-IS (глобально выбраны таймеры: Tx/Rx 100 ms x 3)

Включена аутентификаця для процесса IS-IS (применяется к LSP, CSNP и PSNP)
...
Тип IS-IS интефейса p2p (т.к. все линки являются прямымы стыками)
...
Dсе соседсвта находятся в стосоянии UP, проверяем IS-IS Database (для компактности примет только с DC1-SW3-SPINE1)
...
В IS-IS BD вся необходимая информация о соседях и доступных сетях присутсвует, проверяем таблицу маршрутизации протокола IS-IS
